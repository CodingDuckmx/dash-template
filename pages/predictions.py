...
def processreview(resena):
    resena = cleanning(resena)
    tokenized =  tokenization(resena)
 
    ### Build a function that fills out the word columns:
    
    for i in tokenized.columns.tolist():
        if i in review.columns.tolist():
            resena[i]= resena[i].apply(lambda x: tokenized.loc[0,x]  if x in resena.columns.tolist() else 0)
...
def predict(clicked, text):
    if clicked:
        ...
        text_clean = ' '.join([x for x in text.split() if x in cols])
        review['summary'] = text_clean
        review_tomodel = processreview(review)
